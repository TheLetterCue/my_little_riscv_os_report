...
% figures, diagram of memory split into address spaces, labeled process 0 to 6 and machine memory4
...
% Diagram of space, with stack at the top pointing downwards, and a space for the global pointer at the bottom, possibly next to an illustration of all memory or maybe actual layout of machine space
...
% Find a way to make inline code formatting
...
% Add code section for calculating the address 
% \newpage
\begin{figure}[H]
...
The main way a user may interact with a program in this system will be through the use of a terminal, which requires the need to print and recieve text. To do this, we will utilise one of the two \ac{uart} peripherals. This will allow us to view and send text using a host machine.
\subsubsection{\ac{uart} Config}
